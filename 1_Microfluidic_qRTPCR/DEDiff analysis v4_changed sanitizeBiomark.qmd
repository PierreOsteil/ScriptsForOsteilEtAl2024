...
```{r}
path <- "1_Microfluidic_qRTPCR/rawData/"
...
## Figure 2F
```{r}
cols <- c("FA3" = "red", "FA1" = "rosybrown1", "MB1" = "#37c8ab", "MB3" = "#37c8ab", "MB2" = "#37c8ab")
...
# heatmap
# endo genes Endoderm
EndoGenes <- c("Foxa2", "Sox17", "Krt19", "Hhex",  "Acvr1b", "Gata4", "Hnf4a", "Lgr5",  "Fgf8",  "Cxcr4", "Gata6")
GastruGenes <- c("Mixl1", "T", "Wnt3a", "Nodal", "Lhx1", "Eomes", "Gsc", "Tdgf1", "Axin2", "Tbx6")


TABLE <- DE_WT_noOutlier %>% filter(V2 %in% c("D4")) %>% select(Sample, EndoGenes, V1) %>% mutate(Mean = mean())

TABLE_mat <- as.matrix(TABLE[,2:c(ncol(TABLE)-1)])
rownames(TABLE_mat) <- TABLE[, 1]
mergings <- hclust(dist(TABLE_mat), method = "complete")

heatmap_plot<-heatmaply(percentize(TABLE_mat), Rowv = as.dendrogram(mergings), Row_names_side = "left")
heatmap_plot
ggsave("plots/heatmap.png", plot = heatmap_plot, width = 10, height = 8, dpi = 300)
...
## plot gene expression at Day0
```{r}
#cell line order based on pseudotime
cellLine_order <- c("MB1", "MB2", "MA2", "MB3", "MA1", "FB2", "MA3", "FB1", "FB3", "FA1", "FA3")
cols <- c("FA3" = "red", "FA1" = "magenta", "MB1" = "#37c8ab", "MB3" = "#37c8ab", "MB2" = "#37c8ab", "MA2" = "lightgrey", "MA1" = "lightgrey", "MA3" = "lightgrey", "FB1" = "lightgrey", "FB2" = "lightgrey", "FB3" = "lightgrey")
CellLineNew <- c("MB1" = "C9", "MB2" = "C11", "MA2"= "Eu86", "MB3" = "C16", "MA1" = "Eu79", "FB2" = "C2", "MA3" = "Eu87", "FB1" = "C4", "FB3" = "C3", "FA1" = "C7", "FA3" = "C32")


# plot some genes with Day 0 only
TABLE <- DE_WT_noOutlier_2dCt %>% filter(V2 == "D4")
head(TABLE)
x11()
p <- ggboxplot(TABLE, x = "CellLine", y = "Cxcr4", add = "jitter", fill = "CellLine") +
              scale_x_discrete(limits = cellLine_order)+
              scale_fill_manual(values = cols, guide = F) +
              scale_x_discrete(labels = CellLineNew, limits = cellLine_order)+
              ...
              theme(    axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
                        ...
                        aspect.ratio = 1)
p
ggsave("plots/Boxplot_D0_Mixl1.pdf", p, width = 5, height = 5)
```
...
level_order <- c("D0", "D1", "D2", "D3", "D4")
cols <- c("FA3" = "red", "FA1" = "magenta", "MB1" = "blue", "MB3" = "lightblue", "MB2" = "darkblue", "MA2" = "transparent", "MA1" = "transparent", "MA3" = "transparent", "FB1" = "transparent", "FB2" = "transparent", "FB3" = "transparent")

# Gastrulation + Endoderm
# Transform the y-values of the data frame
p15 <- ggline(TABLE, x = "V2", y = c("Mixl1", "Eomes", "T", "Gsc", "Fgf8", "Mesp1"), combine = T,
              add = c("mean_se"), color = "CellLine", size = 0.5, scales = "free_y") +
              scale_x_discrete(limits = level_order, guide = F)+
              scale_y_continuous(labels = function(x) format(x, scientific = TRUE))+
              labs(x = NULL, y = NULL)+
              scale_colour_manual(values = cols) +
              theme_bw()
p15
ggsave("plots/Timecourse_Gastru-Endo.pdf", p15, width = 12, height = 7 )

# ENDO2
p16 <- ggline(TABLE,  x = "V2", y = c("Sox17", "Foxa2", "Otx2", "Lhx1", "Hhex", "Krt19"), combine = T,
              add = c("mean_se"), color = "CellLine", size = 0.5, scales = "free_y") +
              scale_colour_manual(values = cols) +
              scale_y_continuous(labels = function(x) format(x, scientific = TRUE))+
              scale_x_discrete(limits = level_order, guide = F)+
              ...
              theme_bw()
p16
ggsave("plots/Timecourse_Gastru-Endo2.pdf", p16, width = 12, height = 7)

#MESO
TABLE <- TABLE %>% 
  mutate(across(c("Bmp4", "Myh7", "Klf5", "Kdr", "Mesp1", "Cd34"), ~ 2^.))

p17 <- ggline(TABLE, x = "V2", y = c("Bmp4", "Myh7", "Klf5", "Kdr", "Mesp1", "Cd34"), combine = T,
              add = c("mean_se"), color = "CellLine", size=0.5, scales = "free_y") +
...
#ECTO
TABLE <- TABLE %>% 
  mutate(across(c("Krt10", "Sox1", "Nes", "Foxd3", "Pax6", "Dcx"), ~ 2^.))
...
#PLURI
TABLE <- TABLE %>% 
  mutate(across(c("Pou5f1", "Nanog", "Sox2", "Tdgf1", "Zfp42", "Fgf5"), ~ 2^.))

p19 <- ggline(TABLE, x = "V2", y = c("Pou5f1", "Nanog", "Sox2", "Tdgf1", "Zfp42", "Fgf5"), combine = T,
              add = c("mean_se"), color = "CellLine", size=0.5, scales = "free_y") +
              scale_x_discrete(limits = level_order)+
              scale_colour_manual(values = cols) +
              theme_bw()
...
#ENDOSUPP
TABLE <- TABLE %>% 
  mutate(across(c("Krt19", "Gata4", "AFP", "Axin2" ,"Lgr5", "Nodal"), ~ 2^.))

p20 <- ggline(TABLE, x = "V2", y = c("Krt19", "Gata4", "AFP", "Axin2" ,"Lgr5", "Nodal") , combine = T,
              add = c("mean_se"), color = "CellLine", size=0.5, scales = "free_y") +
              scale_x_discrete(limits = level_order)+
              scale_colour_manual(values = cols) +
              theme_bw()
ggsave("plots/Timecourse_EndoSupp.pdf", p20, width =10, height =7)

#EXTR1
TABLE <- TABLE %>% 
  mutate(across(c("Vim", "Gata4", "AFP", "Axin2" ,"Lgr5", "Nodal"), ~ 2^.))

p20 <- ggline(TABLE, x = "V2", y = c("Vim", "Gata4", "AFP", "Axin2" ,"Lgr5", "Nodal") , combine = T,
              add = c("mean_se"), color = "CellLine", size=0.5, scales = "free_y") +
              scale_x_discrete(limits = level_order)+
              scale_colour_manual(values = cols) +
              theme_bw()
ggsave("plots/Timecourse_Extra.pdf", p20, width =10, height =7)

#EXTR2
TABLE <- TABLE %>% 
  mutate(across(c("Vim", "Pecam1", "Pdgfra" , "Cxcr4"), ~ 2^.))
...
p20 <- ggline(TABLE, x = "V2", y = c("Vim", "Gata4", "AFP", "Axin2" ,"Lgr5", "Nodal") , combine = T,
              ...
              scale_colour_manual(values = cols) +
              theme_bw()
ggsave("plots/Timecourse_Extra2.pdf", p20, width =10, height =7)
...
#DEGs analysis
## ANOVA + Tukey HSD
TABLE_DEGs <- DE_WT_noOutlier %>% filter( V2 == "D0") # on dCt value 
...
sink('stats/ANOVA+Tukey_Day_0.txt')
